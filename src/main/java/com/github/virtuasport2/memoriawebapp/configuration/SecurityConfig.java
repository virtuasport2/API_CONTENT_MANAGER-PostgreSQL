...
public class SecurityConfig {
 ...
 // Definisce un bean che rappresenta la catena di filtri di sicurezza da applicare alle richieste HTTP
 @Bean
 public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
     http
        
         // Disabilita la protezione CSRF: non necessaria per API REST che utilizzano token (JWT)
         ...
         .authorizeHttpRequests(auth -> auth
                 ...
                 .requestMatchers("/api/auth/logout").permitAll()  // Consente l'accesso a /api/auth/logout senza autenticazione    
                 
             // Rende accessibile l'endpoint "/api/auth/login" a chiunque (necessario per il login)

             .requestMatchers("/api/tipi-documento",
            		          "/api/auth/test-logout",
            		          "/api/auth/test-logout1",
            		          "/api/auth/logout",
            		          "/api/auth/login",
            		          "/api/utenti", 
            		          "/api/auth/forgot-password").permitAll()
             // solo utenti già autenticati possano accedere a /forgot-password
             .requestMatchers("/forgot-password","/api/auth/logout").authenticated()
         ...
         // Inserisce il filtro JWT personalizzato prima del filtro standard che gestisce l'autenticazione basata su username/password
         // In questo modo, il filtro JWT ha la possibilità di processare il token e impostare l'autenticazione prima che il filtro standard intervenga
...
//                 .invalidateHttpSession(false) // Poiché stai usando JWT, non invalidare la sessione
//              
//             );
...
//	            //   .requestMatchers("/api/auth/**").permitAll() // Permetti login e registrazione senza token
...
//                               .requestMatchers("/api/documenti","/login", "/resources/**", "/css/**", "/js/**", "/images/**", "/public-api/**")
